...
export const ReportsSchema: initialFValuesTypes = yup.object({ 
 // product_parent_id: yup.number().required("Este campo es requerido"),
  ...
  finishDate : yup.date().required("Este campo es requerido").max(today, "La fecha no puede ser mayor a la actual").min(yup.ref('startDate'),"La fecha no puede ser menor a la inicial"), 
})
